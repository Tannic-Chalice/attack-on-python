...
// --- NEW INTERFACE FOR KYC ---
interface KycData {
  node_id: number;
  identity_score: number;
  signals: Array<{ source: string; status: string; icon: string }>;
  recommendation: string;
}
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} style={{ zIndex: 2000 }}>
      ...
      <style jsx>{`
        ...
        /* Markdown Styles */
        .sar-content h1, .sar-content h2, .sar-content h3 {
          color: #0f172a;
          border-bottom: 2px solid #e2e8f0;
          padding-bottom: 8px;
          margin-top: 24px;
          margin-bottom: 16px;
          font-family: 'Inter', sans-serif; /* Headers in Sans-Serif */
        }
        .sar-content p { margin-bottom: 16px; font-size: 1rem; }
        .sar-content ul { padding-left: 20px; margin-bottom: 16px; }
        .sar-content li { margin-bottom: 8px; }
        .sar-content strong { color: #b91c1c; } /* Highlight key risks in red */
        ...
        .stat-val { display: block; font-size: 1.2rem; font-weight: bold; color: #0f172a; }
        .stat-lbl { font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; }

        .report-modal-content {
            background: #0f172a; /* Dark background behind the paper */
            width: 90vw;
            height: 90vh;
            display: flex;
      ...
      <div className="report-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="header-strip">
          <h2 style={{margin:0, fontSize:'1.2rem'}}>📄 Official Investigation Report</h2>
          <div>
            <button onClick={downloadPDF} style={{marginRight: '10px', background:'#3b82f6', border:'none', padding:'8px 16px', borderRadius:'4px', color:'white', cursor:'pointer'}}>📥 Export PDF</button>
            <button onClick={onClose} style={{background:'#ef4444', border:'none', padding:'8px 16px', borderRadius:'4px', color:'white', cursor:'pointer'}}>Close</button>
          </div>
        </div>

        {/* Scrollable Body */}
        <div className="modal-body" style={{background: '#334155', padding: '40px', overflowY: 'auto'}}>
          
          {/* The "Paper" Document */}
          <div className="paper-document">
            <div style={{textAlign: 'center', marginBottom: '40px', borderBottom: '4px double #000', paddingBottom: '20px'}}>
              <h1 style={{margin: 0, fontSize: '24px', textTransform: 'uppercase', letterSpacing: '2px'}}>Suspicious Activity Report (SAR)</h1>
              <p style={{margin: '10px 0 0 0', color: '#64748b'}}>Confidential • Financial Crimes Enforcement Network</p>
              <div style={{marginTop: '15px', fontSize: '0.9rem'}}>
                <strong>Reference Node:</strong> #{report.node_id} &nbsp;|&nbsp; 
                <strong>Date:</strong> {new Date().toLocaleDateString()}
              </div>
            </div>

            {/* AI Generated Content rendered beautifully */}
            ...
            <div className="stats-grid">
              <div className="stat-box">
                <span className="stat-val">{report.transaction_stats.total_transactions}</span>
                <span className="stat-lbl">Txn Count</span>
              </div>
              <div className="stat-box">
                <span className="stat-val">${report.transaction_stats.total_volume.toLocaleString()}</span>
                <span className="stat-lbl">Total Volume</span>
              </div>
              <div className="stat-box">
                <span className="stat-val">${report.transaction_stats.avg_transaction.toFixed(2)}</span>
                <span className="stat-lbl">Avg Size</span>
              </div>
              <div className="stat-box" style={{borderLeft: '2px solid #ef4444'}}>
                <span className="stat-val" style={{color: '#ef4444'}}>{report.transaction_stats.fraud_count}</span>
                <span className="stat-lbl" style={{color: '#ef4444'}}>Fraud Flags</span>
              </div>
            </div>

            <div style={{marginTop: '40px', textAlign: 'center', fontSize: '0.8rem', color: '#94a3b8'}}>
              <p>Generated by FraudGuard AI • Internal Use Only</p>
            </div>
          </div>
...
const GraphModal = ({ onClose }: { onClose: () => void }) => {
  ...
  // Reset Explanations and KYC data when selecting a new node
  useEffect(() => { 
    setExplanation(null); 
    setExplanationError(null);
    setKycData(null); // RESET KYC
  }, [selectedNode])
  ...
  // --- NEW FEATURE: KYC Verification ---
  const handleKYCDive = async (nodeId: number) => {
    setIsVerifying(true);
    try {
      const res = await axios.get(`${BACKEND_URL}/api/verify_identity/${nodeId}`);
      setKycData(res.data);
    } catch (err) {
      alert("KYC Service Unavailable");
    } finally {
                ...
                <>
                  ...
                  {selectedNode.is_fraud_predicted === 1 && (
                    <div className="explainer-section">
                      ...
                      {/* --- NEW BUTTON: Flag for Investigation --- */}
                      <button 
                  ...
                  {/* --- NEW SECTION: Identity Orchestration (KYC) --- */}
                  <div className="kyc-section" style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px dashed var(--border)' }}>
                    <h4 style={{ color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                      🛡️ Identity Orchestration
                    </h4>
                    
                    {!kycData ? (
                      <button 
                        className="secondary-btn" 
                        onClick={() => handleKYCDive(selectedNode.id)}
                        disabled={isVerifying}
                        style={{ width: '100%', fontSize: '0.8rem', padding: '10px' }}
                      >
                        {isVerifying ? "Fetching External Data..." : "Perform KYC Deep Dive"}
                      </button>
                    ) : (
                      <div className="kyc-results" style={{ background: 'rgba(0,0,0,0.3)', padding: '12px', borderRadius: '8px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                          <span style={{ fontSize: '0.8rem' }}>Trust Score:</span>
                          <span style={{ 
                            fontWeight: 'bold', 
                            color: kycData.identity_score > 70 ? 'var(--accent)' : 'var(--danger)' 
                          }}>
                            {kycData.identity_score}/100
                          </span>
                        </div>

                        <div className="signals-list">
                          {kycData.signals.map((sig: any, i: number) => (
                            <div key={i} style={{ fontSize: '0.75rem', marginBottom: '6px', display: 'flex', justifyContent: 'space-between' }}>
                              <span>{sig.icon} {sig.source}</span>
                              <span style={{ color: sig.status === 'Verified' || sig.status === 'Clear' || sig.status === 'Consistent' ? '#818cf8' : '#fb7185' }}>
                                {sig.status}
                              </span>
                            </div>
                          ))}
                        ...
                        <div style={{ 
                          ...
                          fontSize: '0.7rem', 
                          textAlign: 'center',
                          background: kycData.identity_score < 50 ? 'rgba(239, 68, 68, 0.2)' : 'rgba(16, 185, 129, 0.2)',
                          border: `1px solid ${kycData.identity_score < 50 ? 'var(--danger)' : 'var(--accent)'}`,
                          borderRadius: '4px',
                          fontWeight: 'bold'
                        }}>
                          {kycData.recommendation}
                        </div>
                      </div>
                    )}
                  </div>
      ...
      <div className="container">
        ...
        <main>
          ...
          <div className="view-bar">
            ...
            {/* --- NEW BUTTON: Link to Investigation Workbench --- */}
            <button 
              className="secondary-btn" 
            ...
            <button 
              ...
              style={{ borderColor: '#06b6d4', color: '#06b6d4' }} // Cyan Color for AI
            >
              🤖 Launch FraudGPT Copilot
            </button>

            <button 
  className="secondary-btn" 
  onClick={() => router.push('/GlobalMap')} 
  style={{ borderColor: '#10b981', color: '#10b981' }}
>
  🌍 Geo-Spatial War Room
</button>
          </div>
